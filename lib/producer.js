...
export default function ({ backpack, bakes, slash, target }) {
  return new Promise((resolve, reject) => {
    ...
    multistream((cb) => {
      ...
      if (count === 1) {
        payloadPosition = meter.bytes + 2;
        return cb(undefined, next(
          intoStream(Buffer.from('/*'))
        ));
      } else
      if (count === 2) {
        ...
        if (stripes.length) {
          ...
          if (stripe.buffer) {
            return cb(undefined, pipeToNewMeter(intoStream(Buffer.from(stripe.buffer.toString('base64')))));
          } else
          if (stripe.file) {
            ...
            return cb(undefined, pipeToNewMeter(fs.createReadStream(stripe.file, { encoding: 'base64' })));
          } else {
          ...
          return cb(undefined, next(
            intoStream(Buffer.from(makePreludeBufferFromPrelude(
              replaceDollarWise(
