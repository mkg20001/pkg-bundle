...
export async function exec (argv2) { // eslint-disable-line complexity
  const argv = minimist(argv2, {
    boolean: [ 'b', 'build', 'bytecode', 'd', 'debug',
      'h', 'help', 'public', 'v', 'version', 'm', 'minify', 'debugPrint' ],
  ...
  for (const target of targets) {
    target.binaryPath = path.join(__dirname, '..', 'prelude', 'binary.js');
    target.minify = minify;
  }
  ...
  const backpack = packer({ records, entrypoint, debugPrint: argv.debugPrint });

  targets.forEach((target) => {
    const dataFile = path.basename(target.output);
    target.outputData = path.join(path.dirname(target.output), dataFile + '.data');
